...
MyGame.pieces.player = function (spec) {
  ...
  const moveUp = (elapsedTime) => {
    ...
    if (
      newY - spec.height / 2 > cellHeight * HEIGHT_BOUND &&
      !hitsBarriers({ y: newY, x: spec.center.x })
    ) {
      spec.center.y = newY;
    }
  };
  const moveDown = (elapsedTime) => {
    let newY = spec.center.y + spec.moveRate * elapsedTime;
    if (
      newY + spec.height / 2 < canvas.height &&
      !hitsBarriers({ y: newY, x: spec.center.x })
    ) {
      spec.center.y = newY;
    }
  ...
  const moveRight = (elapsedTime) => {
    ...
    if (
      newX + spec.width / 2 < canvas.width &&
      !hitsBarriers({ y: spec.center.y, x: newX })
    ) {
      spec.center.x = newX;
    }
  };
